...
class Database:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO request_logs (
                    ...
                    proxy_used, proxy_url_display, proxy_ip, proxy_country, proxy_country_code, proxy_details
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                log.token_id,
                ...
                log.duration,
                log.proxy_used,
                log.proxy_url_display,
                log.proxy_ip,
                log.proxy_country,
                log.proxy_country_code,
                log.proxy_details,
            ))
            await db.commit()
            return cursor.lastrowid
            ...
            if proxy_data is not None:
                ...
                for field in proxy_fields:
                    if field in proxy_data:
                        ...
                        params.append(proxy_data.get(field))
    ...
    async def update_request_log_task_id(self, log_id: int, task_id: str,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            updates = ["task_id = ?", "updated_at = CURRENT_TIMESTAMP"]
            ...
            if proxy_data is not None:
                ...
                for field in proxy_fields:
                    if field in proxy_data:
                        updates.insert(-1, f"{field} = ?")
                        params.append(proxy_data.get(field))

            params.append(log_id)
            await db.execute(
                f"UPDATE request_logs SET {', '.join(updates)} WHERE id = ?",
                params
            )
            await db.commit()
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            cursor = await db.execute("""
                SELECT
                    ...
                    rl.duration,
                    rl.proxy_used,
                    rl.proxy_url_display,
                    rl.proxy_ip,
                    rl.proxy_country,
                    rl.proxy_country_code,
                    rl.proxy_details,
                    rl.created_at,
