...
class SoraClient:
    ...
    async def _record_proxy_trace(
        self,
        proxy_trace: Optional[List[Dict[str, Any]]],
        scope: str,
        token_id: Optional[int] = None,
        proxy_url: Optional[str] = None,
        use_image_upload_proxy: bool = False,
    ) -> None:
        """Record the effective proxy snapshot for the current top-level request."""
        if proxy_trace is None:
            return

        snapshot = await self.proxy_manager.resolve_proxy_snapshot(
            scope=scope,
            token_id=token_id,
            proxy_url=proxy_url,
            use_image_upload_proxy=use_image_upload_proxy,
        )
        self.proxy_manager.merge_proxy_trace(proxy_trace, snapshot)
    ...
    async def _nf_create_urllib(
        ...
        cookie_header: Optional[str] = None,
        proxy_trace: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        ...
        await self._record_proxy_trace(
            proxy_trace,
            scope="generation",
            token_id=token_id,
            proxy_url=proxy_url,
    ...
    async def _make_request(self, method: str, endpoint: str, token: str,
                           ...
                           use_image_upload_proxy: bool = False,
                           proxy_trace: Optional[List[Dict[str, Any]]] = None,
                           proxy_scope: str = "request") -> Dict[str, Any]:
        ...
        await self._record_proxy_trace(
            proxy_trace,
            scope="image_upload" if use_image_upload_proxy else proxy_scope,
            token_id=token_id,
            proxy_url=proxy_url,
            use_image_upload_proxy=use_image_upload_proxy,
        )
    ...
    async def generate_image(self, prompt: str, token: str, width: int = 360,
                            ...
                            token_id: Optional[int] = None,
                            proxy_trace: Optional[List[Dict[str, Any]]] = None) -> str:
    ...
    async def get_image_tasks(self, token: str, limit: int = 20,
                              token_id: Optional[int] = None,
                              proxy_trace: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    async def get_pending_tasks(self, token: str, token_id: Optional[int] = None,
                                proxy_trace: Optional[List[Dict[str, Any]]] = None) -> list:
    ...
    async def post_video_for_watermark_free(self, generation_id: str, prompt: str, token: str,
                                            proxy_trace: Optional[List[Dict[str, Any]]] = None) -> str:
