...
async def get_logs(limit: int = 100, token: str = Depends(verify_admin_token)):
    ...
    for log in logs:
        ...
        proxy_details_raw = log.get("proxy_details")
        proxy_details = []
        if proxy_details_raw:
            try:
                proxy_details = json.loads(proxy_details_raw)
            except (TypeError, json.JSONDecodeError):
                proxy_details = []

        proxy_used = log.get("proxy_used")
        ...
        log_data = {
            ...
            "proxy_url_display": log.get("proxy_url_display"),
            "proxy_ip": log.get("proxy_ip"),
            "proxy_country": log.get("proxy_country"),
            "proxy_country_code": log.get("proxy_country_code"),
